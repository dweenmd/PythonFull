...
class User(Account):
    ...
    def transfer(self, other_account, amount):
        if amount <= self.balance:
            ...
            self.add_transaction(f"Transfer: -{amount} to {other_account.name}")
            other_account.add_transaction(f"Transfer: +{amount} from {self.name}")
    ...
    def request_loan(self, amount):
        if amount <= 2 * self.balance:
            self.loan += amount
            self.deposit(amount)  # Deposit loan amount directly into the account
